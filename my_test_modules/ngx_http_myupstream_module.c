...
static void * ngx_http_myupstream_loc_conf(ngx_conf_t *cf);
...
static ngx_http_module_t  ngx_http_myupstream_module_ctx = {
    ...
    ngx_http_myupstream_loc_conf,              /* create location configuration */
...
ngx_int_t ngx_http_myupstream_handler(ngx_http_request_t *r) {
...
static ngx_int_t myupstream_upstream_create_request(ngx_http_request_t *r) {
  ...
  ngx_int_t queryLineLen = backendQueryLine.len + r->args.len - 2;  //why - 2?
...
static ngx_int_t myupstream_upstream_process_header(ngx_http_request_t *r) {
  ...
  for (;;) {
    ...
    if (rc == NGX_OK) {
      ...
      h->key.data = ngx_pnalloc(r->pool, h->key.len + 1 + h->value.len + 1 + h->key.len);     //why this len?
      if (h->key.data == NULL)
        return NGX_ERROR;
...
//
//static ngx_int_t
//ngx_http_myupstream_init(ngx_conf_t *cf)
//{
//  ngx_http_top_body_filter = ngx_http_myupstream;
//
//  return NGX_OK;
//}
