...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    //?????????????? cycle->conf_ctx ?????????????
    //????core module?configure.???????core module?create_conf?????????conf?
    //?????conf????????conf_ctx?
    ...
    conf.pool = pool;
    conf.log = log;
    conf.module_type = NGX_CORE_MODULE;//?????????????MAIN????????core?
    ...
    //????????????????????????????????
    //????????ngx_command_t????????????set???????????ngx_conf_parse???????.
